test_that("plot_compass works", {
  ...
  rm_fn <- function(.fn_vec = fn_vec){
    for(x in .fn_vec) invisible(suppressWarnings(file.remove(file.path(tempdir(), x))))
  }
  plot_compass(c_obj_list, dir_save = tempdir(), type = 'pp', return_plot_list = FALSE,
               shift_plot_pp_y = -0.07, shift_plot_heatmap_x = 0.034)
  expect_true(file.exists(file.path(tempdir(), "compass_boxplots_grid.png")))
  rm_fn()
  data('c_obj_list', package = 'compassutils')
  plot_compass(c_obj_list, dir_save = tempdir(), type = 'pp', return_plot_list = FALSE,
               shift_plot_pp_y = -0.07, shift_plot_heatmap_x = 0.034, save_ind = TRUE,
               save_grid = FALSE)
  expect_true(!file.exists(file.path(tempdir(), "compass_boxplots_grid.png")))
  ...
  plot_compass(c_obj_list[1], dir_save = tempdir(), type = c('pp', 'scores'),
  ...
  plot_compass(c_obj_list, dir_save = tempdir(), type = c('pp', 'scores'),
               return = FALSE, shift_plot_scores = c(-0.05, 0.05), facet = FALSE,
               shift_plot_pp_y = -0.05, shift_plot_heatmap_x = 0.052, file_grid = 'alternate',
               save_grid = FALSE, save_ind = TRUE, file_ind = setNames(c('a', 'b', 'c', 'd'),
