#' @param ind logical. If \code{TRUE}, then sets of posterior probability and PFS plots are saved individually.
#' @param p_list_pp list of \code{ggplot2} objects. Posterior probability plots and associated
#' cytokine-label grids, as returned by \code{compassutils:::.plot_compass_pp}.
#' @param p_list_scores list of \code{ggplot2} objects. PFS and FS response plots, as
...
.save_layout <- function(p, ind, p_list_pp, p_list_scores = NULL,
  ...
  for(i in seq_along(p_list_pp$p_probs)){
    ...
    row_ind <- j %/% n_col + j %% n_col
    col_ind <- j - (row_ind - 1) * n_col
...
#'
.save_plot <- function(p, height, width, dir_save, file, save_format, n_row, n_col){
  if(is.null(height)){
